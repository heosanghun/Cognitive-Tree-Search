...
- **OpenMathInstruct-1** (`nvidia/OpenMathInstruct-1`): streaming subset -> JSONL
...
def download_openmath_subset(out_dir: Path, n_rows: int) -> Path:
    stream = load_dataset("nvidia/OpenMathInstruct-1", split="train", streaming=True)
...
def main() -> None:
    ...
    ap.add_argument("--openmath-rows", type=int, default=100_000, help="OpenMathInstruct-1 streaming cap")
